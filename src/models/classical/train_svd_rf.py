...
def train_and_evaluate(
    ...
    print(
        f"[INFO] Reduced train matrix: "
        f"{x_train.shape}"
    )

    print(
        f"[INFO] Explained variance ratio: "
    ...
    val_mean_macro_f1 = sum(
        results[task]["val"]["macro_f1"]
        for task in TASKS
    ) / len(TASKS)

    test_mean_macro_f1 = sum(
        results[task]["test"]["macro_f1"]
        for task in TASKS
    ) / len(TASKS)

    summary = {
        "model": "tfidf_svd_random_forest",
        "tasks": TASKS,
        "tfidf": {
            "ngram_range": [1, 2],
            "min_df": 2,
